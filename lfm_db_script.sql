...
-- select coalesce(max(uploaded_at)::timestamp,'2005-01-01 00:00:00+00'::timestamp) from raw_files;

-- select count(*)/(count( distifile_id)) from log_entries;

-- select t.team_name,count(*) as total_uploads from raw_files rf join teams t on rf.team_id=t.team_id group by t.team_name;

-- select uploaded_at::date as date,count(*) as total_files from raw_files group by date order by date;

-- select u.username, u.first_name, u.last_name, u.phone_no, u.email, u.gender, r.role_name, t.team_name, u.created_at from users u join teams t on u.team_id=t.team_id join roles r on r.role_id=u.role_id

-- select round(count(*)/NULLIF((count( distinct rf.file_id)),0),2) from log_entries le join raw_files rf on le.file_id=rf.file_id where rf.is_archived=false;
...
-- TRUNCATE TABLE raw_files RESTART IDENTITY CASCADE;
